export function VendorsListSkeleton() {
  return (
    <div className="space-y-3" aria-busy="true" aria-label="กำลังโหลดผู้ขาย">
        ...
        <ul className="divide-y divide-border">
          {Array.from({ length: 5 }).map((_, index) => (
            <li key={index} className="flex items-center gap-4 px-4 py-3.5">
              ...
              <div className="hidden h-4 w-20 animate-pulse rounded bg-surface motion-reduce:animate-none md:block" />
              <div className="h-4 w-8 animate-pulse rounded bg-surface motion-reduce:animate-none" />
              <div className="h-5 w-16 shrink-0 animate-pulse rounded-full bg-surface motion-reduce:animate-none" />
      ...
      <span className="sr-only">กำลังโหลดผู้ขาย...</span>
    </div>
  );
}
