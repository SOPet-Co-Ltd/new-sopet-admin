...
export function getVendorOrders(storeId: string): Promise<Order[]> {
  return executeQuery<{ vendorOrders: Parameters<typeof mapOrder>[0][] }>(VENDOR_ORDERS_QUERY, {
    storeId,
  }).then((data) => data.vendorOrders.map(mapOrder));
}
